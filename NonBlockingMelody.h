...
class NonBlockingMelody {
  ...
  public:
    ...
    void play(const Note *notes, uint16_t length, uint16_t repeats = 1); // Play a melody
    void pause();                                                        // Pause the melody
    void resume();                                                       // Resume the melody
    void stop();                                                         // Stop the melody
    bool playing(const Note *notes = nullptr);                           // Returns the playback status
  ...
  private:
    ...
    const uint8_t _speakerPin;        // The pin the speaker is connected to
    bool          _playing;           // Playback status flag
    const Note    *_notes;            // Pointer to the melody
